...
export const TopKpiSummaryBar: React.FC<TopKpiSummaryBarProps> = ({
  ...
  agingConfig = {
    ...
    customRuleName: 'LGE Standard (14/30 Days)'
  },
  onSelectFilter,
  onNavigateToLayout,
  onZoneChange
    ...
    <div className="w-full shrink-0 animate-fadeIn space-y-1.5">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-2.5 lg:gap-3.5">
        ...
        <div 
          ...
          title={`${t('kpi.totalBalance')} (${kpiData.zoneName})`}
        >
          <div className="flex items-center justify-between leading-none">
            <div className="flex items-center space-x-1.5 min-w-0">
              <span className="text-xs sm:text-[13px] font-black text-slate-100 tracking-tight truncate">
                {currentZoneKey === 'ALL' ? t('kpi.totalBalance') : `${t('kpi.totalBalanceZone')} ${kpiData.zoneShortName}`}
              </span>
            </div>
          ...
          <div className="flex items-baseline my-0.5 leading-none">
            ...
            <span className="ml-1.5 text-xs text-slate-300 font-extrabold shrink-0">{t('common.units')}</span>
          </div>
          <div className="flex items-center justify-between text-[10.5px] font-black leading-tight">
            <div className="flex items-center space-x-1 text-emerald-400 truncate">
              ...
              <span className="truncate">{currentZoneKey === 'ALL' ? t('common.realTimeSync') : `${t('common.zone')} ${kpiData.badgeLabel}`}</span>
            </div>
            {currentZoneKey !== 'ALL' && (
              <span className="text-[10px] text-blue-400 font-bold bg-blue-950/60 px-1.5 py-0.5 rounded border border-blue-800/60">
                {kpiData.occupiedPallets} {t('common.pallets')}
              </span>
            )}
          </div>
        </div>
        ...
        <div 
          ...
          title={`${t('kpi.safetyStockAlert')} (${kpiData.zoneName})`}
        >
          <div className="flex items-center justify-between leading-none">
            <span className="text-xs sm:text-[13px] font-black text-slate-100 tracking-tight truncate">
              {t('kpi.safetyStockAlert')}
            </span>
          ...
          <div className="flex items-baseline my-0.5 leading-none">
            ...
            <span className="ml-1.5 text-xs text-slate-300 font-extrabold shrink-0">{t('common.items')}</span>
          </div>
          <div className="text-[10.5px] font-black text-rose-400 truncate leading-tight">
            {t('kpi.belowThreshold')} {currentZoneKey !== 'ALL' ? `(${kpiData.zoneShortName})` : ''}
          </div>
        </div>
        ...
        <div 
          ...
          title={`${t('kpi.todayScanIn')} (${kpiData.zoneName})`}
        >
          <div className="flex items-center justify-between leading-none">
            <span className="text-xs sm:text-[13px] font-black text-slate-100 tracking-tight truncate">
              {t('kpi.todayScanIn')}
          ...
          <div className="flex items-baseline my-0.5 leading-none">
            ...
            <span className="ml-1.5 text-xs text-slate-300 font-extrabold shrink-0">{t('kpi.scansCount')}</span>
          </div>
        ...
        <div 
          ...
          title={`${t('kpi.todayScanOut')} (${kpiData.zoneName})`}
        >
          <div className="flex items-center justify-between leading-none">
            <span className="text-xs sm:text-[13px] font-black text-slate-100 tracking-tight truncate">
              {t('kpi.todayScanOut')}
        ...
        <div 
          ...
          title={`${t('kpi.storageOccupancy')} (${kpiData.zoneName})`}
        >
          <div className="flex items-center justify-between leading-none">
            <span className="text-xs sm:text-[13px] font-black text-slate-100 tracking-tight truncate">
              {t('kpi.storageOccupancy')}
            </span>
          ...
          <div className="flex items-center justify-between text-[10.5px] font-bold leading-tight truncate">
            <span className="text-slate-300 truncate">{t('warehouse.utilization')}:</span>
        ...
        <div 
          ...
          title={`${t('kpi.fifoAgingAlert')} (${kpiData.zoneName})`}
        >
          <div className="flex items-center justify-between leading-none">
            <span className="text-xs sm:text-[13px] font-black text-slate-100 tracking-tight truncate">
              {t('kpi.fifoAgingAlert')}
            </span>
          ...
          <div className="flex items-baseline my-0.5 leading-none">
            ...
            <span className="ml-1.5 text-xs text-slate-300 font-extrabold shrink-0">{t('common.overdue')}</span>
          </div>
          <div className="text-[10.5px] font-bold text-amber-400 truncate leading-tight">
            &gt; {agingConfig?.criticalDays || 28} {t('warehouse.agingDays')}
          </div>
        </div>
